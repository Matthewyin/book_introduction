# COVER-HANDOFF — ~~封面流程说明~~（已废弃）

> ⚠️ **本文件已废弃**（2026-08）：封面流程已统一改为 `scripts/cover-shier.py`
> （视频截图 + LXGW 手写体白色文字，复刻「十二..」风格，A/B 线通用）。
> 下面的旧 cover-compose.py / AI 底图模板 / 双风格轨内容仅保留作历史参考，
> **后续会话不要再走 cover-compose.py 路线**。
