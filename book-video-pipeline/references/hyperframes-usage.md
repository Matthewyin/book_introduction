...
相邻帧之间使用**交叉淡入淡出**（cross-fade），通过每帧背景图层的 `opacity` 动画实现平滑过渡，禁止硬切。

| 参数 | 值 |
|------|-----|
| 转场类型 | 交叉淡入淡出 |
| 作用层 | 每帧 bg 层（`opacity` 动画） |
| 时长 | 0.6s |
| 缓动 | `power1.inOut` |
| 实现方式 | GSAP：新帧 `opacity 0→1` + 旧帧 `opacity 1→0`，在转场窗口内重叠 |
| 首帧 | 不淡入（直接显示） |

```js
// 非首帧场景淡入
tl.fromTo("#scene-N", { opacity: 0 }, { opacity: 1, duration: 0.6, ease: "power1.inOut" }, start);
```

> 注意：i2v 视频帧不参与交叉淡入（视频本身有自己的播放过渡），仅图片背景帧使用。
...
| 金句浮起 | ink：`scale 0.8→1` + `opacity 0→1`，0.8s，`back.out`；note：`y 16→0` + `opacity 0→1`，0.7s，`power2.out` |
...
|------|--------|------|------|------|----------|
| **ink**（墨色） | 书页、概念镜 | 宋体/楷体 | 深墨色 `#2e1f10` | **透明**（靠书页本身做底） | **缩放弹入**：`scale 0.8→1` + `opacity 0→1`，0.8s，`ease: back.out` |
| **note**（便签） | 实景镜 | 黑体 | 白色 `#fff` + 3px 黑描边 | **透明** | **上滑浮入**：`opacity 0→1` + `y 16→0`，0.7s，`ease: power2.out` |
...
| 类型 | 用在哪 | 样式 | 出现动画 |
|------|--------|------|----------|
| **ink**（墨色缩放弹入） | 书页、概念镜 | 无底纹，直接浮在画面上 | `scale 0.8→1` + `opacity 0→1`，0.8s，`back.out` |
| **note**（便签上滑浮入） | 实景镜 | 米色纸块 + washi tape + 轻微旋转（±2°），贴角落 | `opacity 0→1` + `y 16→0`，0.7s，`power2.out` |
